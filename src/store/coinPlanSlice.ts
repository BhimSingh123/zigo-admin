...
export const getCoinPlanUserHistory: any = createAsyncThunk(
    ...
    async (payload: any) => {
        ...
        return apiInstanceFetch.get(`api/admin/history/getCoinTransactionHistory?userId=${payload?.id}&start=${payload?.start}&limit=${payload?.limit}&startDate=${payload?.startDate}&endDate=${payload?.endDate}&search=${search}`);
    }
);
...
export const getPlanPurchaseHistory: any = createAsyncThunk(
    ...
    async (payload: { start: number; limit: number; startDate: string; endDate: string; type: number | string;  search: string; paymentGateway?: string } | undefined) => {
        return apiInstanceFetch.get(
            `api/admin/coinPlan/retrieveCoinPlanPurchase?start=${payload?.start}&limit=${payload?.limit}&startDate=${payload?.startDate}&endDate=${payload?.endDate}&type=${payload?.type}&search=${payload?.search}&paymentGateway=${payload?.paymentGateway || ""}`
        );
    }
);
...
export const retrieveCoinPlanPurchase: any = createAsyncThunk(
    ...
    async (payload: { start: number; limit: number; startDate: string; endDate: string; type: number; userId: string; search: string } | undefined) => {
        const search = encodeURIComponent(payload?.search ?? "");
        return apiInstanceFetch.get(
            `api/admin/coinPlan/retrieveCoinPlanPurchase?start=${payload?.start}&limit=${payload?.limit}&startDate=${payload?.startDate}&endDate=${payload?.endDate}&type=${payload?.type}&userId=${payload?.userId}&search=${search}`
        );
    }
);
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(
            ...
            (state, action: PayloadAction<any>) => {
                ...
                state.totalPlanPurchaseHistory = action.payload.totalRecords;
