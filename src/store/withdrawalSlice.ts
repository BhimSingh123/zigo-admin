...
export const getWithdrawalRequest: any = createAsyncThunk(
  ...
  async (payload: AllUsersPayload | undefined) => {
    const searchParam =
      (payload as any)?.search && (payload as any)?.search !== "All"
        ? (payload as any)?.search
        : "";
    return await apiInstanceFetch.get(
      `api/admin/withdrawalRequest/retrievePayoutRequests?start=${payload?.start}&limit=${payload?.limit}&startDate=${payload?.startDate}&endDate=${payload?.endDate}&status=${payload?.status}&person=${payload?.person}&search=${encodeURIComponent(
        searchParam
      )}`
    );
  }
