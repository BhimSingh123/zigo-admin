...
/** Host info: followers / blocked users — same query shape as fetchHostList (hostId + start + limit + search). */
export type HostRelationListPayload =
  | string
  | {
      hostId: string;
      start?: number;
...
function normalizeHostRelationPayload(
  payload: HostRelationListPayload | undefined
): { hostId: string; start?: number; limit?: number; search?: string } | null {
  if (payload == null || payload === "") return null;
  if (typeof payload === "string") {
    return { hostId: payload };
  }
  return {
...
function buildHostFollowersQuery(
  p: { hostId: string; start?: number; limit?: number; search?: string }
) {
  const baseUrl = "api/admin/followerFollowing/fetchFollowers";
  const params = new URLSearchParams();
  params.append("hostId", p.hostId);
...
function buildBlockedUsersForHostQuery(
  p: { hostId: string; start?: number; limit?: number; search?: string }
) {
  const baseUrl = "api/admin/block/listBlockedUsersForHost";
  const params = new URLSearchParams();
  params.append("hostId", p.hostId);
  if (p.start !== undefined) params.append("start", String(p.start));
  if (p.limit !== undefined) params.append("limit", String(p.limit));
...
export const getRealOrFakeHost: any = createAsyncThunk(
  ...
  async (payload: AllImpressionPayload | undefined) => {
    const baseUrl = "api/admin/host/fetchHostList";
    const params = new URLSearchParams();
    ...
    if (payload?.type !== undefined) params.append("type", String(payload.type));
...
export const getHostFollowerList: any = createAsyncThunk(
  ...
  async (payload: HostRelationListPayload | undefined) => {
    const p = normalizeHostRelationPayload(payload);
    if (!p?.hostId) {
      return apiInstanceFetch.get("api/admin/followerFollowing/fetchFollowers");
    }
    return apiInstanceFetch.get(buildHostFollowersQuery(p));
  }
);
...
export const getUserBlockList: any = createAsyncThunk(
  ...
  async (payload: HostRelationListPayload | undefined) => {
    const p = normalizeHostRelationPayload(payload);
    if (!p?.hostId) {
      return apiInstanceFetch.get("api/admin/block/listBlockedUsersForHost");
    }
    return apiInstanceFetch.get(buildBlockedUsersForHostQuery(p));
  }
);
...
export const getCoinPlanHistory: any = createAsyncThunk(
  ...
  async (payload: any) => {
    ...
    return apiInstanceFetch.get(
      `api/admin/history/fetchCoinTransactionHistory?hostId=${payload?.id}&startDate=${payload?.startDate}&endDate=${payload?.endDate}&start=${payload?.start}&limit=${payload?.limit}&search=${search}`
    );
  }
);
...
export const getHostCallHistory: any = createAsyncThunk(
  ...
  async (payload: any) => {
    const search = encodeURIComponent(payload?.search ?? "");
    return apiInstanceFetch.get(
      `api/admin/history/listCallTransactions?hostId=${payload?.id}&startDate=${payload?.startDate}&endDate=${payload?.endDate}&start=${payload?.start}&limit=${payload?.limit}&search=${search}`
    );
  }
);
...
export const getHostGiftHistory: any = createAsyncThunk(
  ...
  async (payload: any) => {
    const search = encodeURIComponent(payload?.search ?? "");
    return apiInstanceFetch.get(
      `api/admin/history/fetchGiftTransactionHistory?hostId=${payload?.id}&startDate=${payload?.startDate}&endDate=${payload?.endDate}&start=${payload?.start}&limit=${payload?.limit}&search=${search}`
    );
  }
);
...
const hostSlice = createSlice({
  ...
  initialState,
  reducers: {
    clearHostFollowerList: (state) => {
      state.hostFollowerList = [];
      state.totalFollowerList = 0;
      state.isSkeleton = false;
    },
    clearUserBlockList: (state) => {
      state.userBlockList = [];
      state.totalUserBlockList = 0;
      state.isSkeleton = false;
    },
  },
  extraReducers: (builder) => {
    ...
    builder.addCase(getUserBlockList.fulfilled, (state, action) => {
      ...
      state.totalUserBlockList =
        action?.payload?.total ?? action?.payload?.totalUserBlock ?? 0;
    });
    ...
    builder.addCase(getHostFollowerList.fulfilled, (state, action) => {
      ...
      state.totalFollowerList =
        action?.payload?.total ?? action?.payload?.totalFollower ?? 0;
    });
...
export const { clearHostFollowerList, clearUserBlockList } = hostSlice.actions;
