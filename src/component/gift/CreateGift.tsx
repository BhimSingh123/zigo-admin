...
const CreateGift = () => {
  ...
  const originalCoin = String(dialogueData?.giftAll?.coin ?? "").trim();
  const originalGiftCategoryId = String(
    dialogueData?.giftData?._id ?? ""
  ).trim();
  const originalImagePath = dialogueData?.giftAll?.image
    ? baseURL + dialogueData.giftAll.image.replace(/\\/g, "/")
    : "";

  const currentCoin = String(coin ?? "").trim();
  const currentGiftCategoryId = String(giftCategoryId ?? "").trim();
  const currentImagePath = String(imagePath ?? "").trim();
  const hasChanges =
    !isEdit ||
    currentCoin !== originalCoin ||
    currentGiftCategoryId !== originalGiftCategoryId ||
  ...
  const handleSubmit = (e: any) => {
    ...
    const isUpdate = Boolean(mongoId);
