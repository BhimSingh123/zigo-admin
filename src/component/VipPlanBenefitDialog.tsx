...
const VipPlanBenefitDialog = () => {
  ...
  const originalAudioCallDiscount = String(
    dialogueData?.audioCallDiscount ?? ""
  ).trim();
  const originalVideoCallDiscount = String(
    dialogueData?.videoCallDiscount ?? ""
  ).trim();
  const originalRandomMatchCallDiscount = String(
    dialogueData?.randomMatchCallDiscount ?? ""
  ).trim();
  const originalTopUpCoinBonus = String(dialogueData?.topUpCoinBonus ?? "")
    .trim();
  const originalFreeMessages = String(dialogueData?.freeMessages ?? "")
    .trim();

  const currentAudioCallDiscount = String(audioCallDiscount ?? "").trim();
  const currentVideoCallDiscount = String(videoCallDiscount ?? "").trim();
  const currentRandomMatchCallDiscount = String(
    randomMatchCallDiscount ?? ""
  ).trim();
  const currentTopUpCoinBonus = String(topUpCoinBonus ?? "").trim();
  const currentFreeMessages = String(freeMessages ?? "").trim();

  const hasChanges =
    !isEdit ||
    currentAudioCallDiscount !== originalAudioCallDiscount ||
    currentVideoCallDiscount !== originalVideoCallDiscount ||
    currentRandomMatchCallDiscount !== originalRandomMatchCallDiscount ||
    currentTopUpCoinBonus !== originalTopUpCoinBonus ||
    currentFreeMessages !== originalFreeMessages ||
    Boolean(image);
