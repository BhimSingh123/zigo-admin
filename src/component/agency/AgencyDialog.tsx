...
const AgencyDialog = () => {
    ...
    const hasChanges = useMemo(() => {
        ...
        const originalName = String(dialogueData?.name ?? "").trim();
        if (currentName !== originalName) return true;

        const currentEmail = String(email ?? "")
            .trim()
            .toLowerCase();
        const originalEmail = String(dialogueData?.email ?? "")
            .trim()
            .toLowerCase();
        if (currentEmail !== originalEmail) return true;

        const currentCommission = String(commission ?? "").trim();
        const originalCommission = String(dialogueData?.commission ?? "").trim();
        if (currentCommission !== originalCommission) return true;

        const currentMobileNumber = String(mobileNumber ?? "").trim();
        const originalMobileNumber = String(
            dialogueData?.mobileNumber ?? ""
        ).trim();
        if (currentMobileNumber !== originalMobileNumber) return true;
        ...
        const currentCountryCode = String(countryCode ?? "").trim();
        const originalCountryCode = String(
            dialogueData?.countryCode ?? ""
        ).trim();
        if (currentCountryCode !== originalCountryCode) return true;

        const passwordValue = String(password ?? "").trim();
        const originalPassword = String(dialogueData?.password ?? "");
        const passwordChanged =
            passwordValue.length > 0 && passwordValue !== originalPassword;
        if (passwordChanged) return true;

        if (image) return true;

        const currentCountryName = String(selectedCountry?.name ?? "");
        const originalCountryName = String(dialogueData?.country ?? "");
        if (currentCountryName !== originalCountryName) return true;

        const currentCountryFlag = String(selectedCountry?.flag ?? "");
        const originalCountryFlag = String(dialogueData?.countryFlagImage ?? "");
        if (currentCountryFlag !== originalCountryFlag) return true;
    ...
    useEffect(() => {
        ...
        setCountryCode(dialogueData?.countryCode ? String(dialogueData.countryCode) : "");
    }, [countryData]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!commission) {
                newError.commission = "Commission is required";
            } else {
                const numericCommission = Number(commission);
                if (Number.isNaN(numericCommission)) {
                    newError.commission = "Commission must be a number between 0 and 100";
                } else if (numericCommission <= 0) {
                    newError.commission = "Commission can not be less than or equal to 0";
                } else if (numericCommission > 100) {
                    newError.commission = "Commission can not be greater than 100";
                }
            }
                                    ...
                                    <div className="col-6">
                                        <ExInput
                                            ...
                                            onChange={(e: any) => {
                                                ...
                                                if (numericCommission <= 0) {
                                                    return setError({
                                                        ...error,
                                                        commission:
                                                            "Commission can not be less than or equal to 0",
                                                    });
                                                }

                                                if (numericCommission > 100) {
                                                    return setError({
                                                        ...error,
                                                        commission:
                                                            "Commission can not be greater than 100",
                                                    });
                                                }
