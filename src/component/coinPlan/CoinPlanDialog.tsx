...
const CoinPlanDialog = () => {
    ...
    const originalCoin = String(dialogueData?.coins ?? "").trim();
    const originalBonusCoin = String(dialogueData?.bonusCoins ?? "").trim();
    const originalPrice = String(dialogueData?.price ?? "").trim();
    const originalProductId = String(dialogueData?.productId ?? "").trim();

    const currentCoin = String(coin ?? "").trim();
    const currentBonusCoin = String(bonusCoin ?? "").trim();
    const currentPrice = String(price ?? "").trim();
    const currentProductId = String(productId ?? "").trim();

    const hasChanges =
        !isEdit ||
        currentCoin !== originalCoin ||
        currentBonusCoin !== originalBonusCoin ||
        currentPrice !== originalPrice ||
        currentProductId !== originalProductId;
